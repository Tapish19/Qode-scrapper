...
Sign in to X manually.

Confirm that the X home page loads.

Completely close that dedicated Chrome window.

Configure the same profile path in .env.
...
Opens Chrome using the configured persistent profile.

Loads each query from config/search_queries.txt.

Opens the X Latest search timeline.

Extracts visible posts incrementally.

Rejects posts outside the exact rolling 24-hour window.

Normalizes text and engagement values.

Deduplicates using tweet IDs and fingerprints.

Writes accepted records to partitioned Parquet files.

Continues until the cumulative target is reached or available results are exhausted.
...
Aggregates signals into fifteen-minute windows.

Keeps one accumulator per market/time window.

Samples aggregate points only when the chart exceeds the configured limit.

Writes the final visualization without keeping the full raw dataset in plotting memory.
...
Analysis-ready normalized text

language_hint

string

Indic, English, mixed, or unknown heuristic
...
Extracted account mentions

hashtags

list[string]

Extracted normalized hashtags
...
Search query that found the post
...
Parquet date partition

event_hour

string

Parquet hour partition
...
Collect Indian market discussions

Selenium X collector

Minimum 2,000 posts

Cumulative rolling-window target

Previous 24 hours

X date query plus exact Python cutoff
...
Stored as typed fields

Mentions and hashtags

Extracted into list columns

Efficient processing

Incremental and batch-based pipeline

Rate-limit/error handling

Delays, retries, stopping conditions, logging
...
In-memory, SQLite, and dataset-aware checks

Unicode handling

NFKC normalization with Indic preservation

Text-to-signal conversion

Sparse vectors plus custom finance features

Memory-efficient plotting

Fifteen-minute aggregation and bounded sampling

Composite signals

Weighted bounded signal
...
Partitioned storage and replaceable analytics

Documentation

README and technical design
...
See docs/technical_design.md for architecture details, trade-offs, failure handling, and the scalability plan.
