...
## Optional Synthetic Test Workflow

Synthetic data is included only for testing and reproducibility.

> Synthetic posts do not satisfy the assignment requirement for collecting 2,000 real X posts.
...
```powershell
python -m market_intel generate-sample --count 2000 --output data/sample/synthetic/tweets
```
...
python -m market_intel generate-sample \
  --count 2000 \
  --output data/sample/synthetic/tweets
```
...
```powershell
python -m market_intel analyze --input data/sample/synthetic/tweets --output data/output_synthetic
```
...
```bash
python -m market_intel analyze \
  --input data/sample/synthetic/tweets \
  --output data/output_synthetic
```

Synthetic outputs are kept separate from real outputs:

```text
data/output_synthetic/
```

## Sample Artifacts for GitHub

For a public repository, include a limited real verification sample rather than necessarily publishing the complete 2,000-post dataset.

Recommended sample artifacts:

```text
data/sample/real_tweets_sample.csv
data/sample/collection_manifest.json
data/output_real/signals_15m.csv
data/output_real/summary.json
data/output_real/signals.png
```

The real sample should retain:
...
This allows reviewers to open a sample of original URLs and independently verify that the records came from real X posts.

The complete Parquet dataset may be included when the repository is private or supplied separately to the evaluator.
...
A collection manifest should record:

```json
{
  "dataset_type": "live_x_collection",
  "is_synthetic": false,
  "collector": "selenium",
  "target_records": 2000,
  "window_hours": 24,
  ...
  "deduplication": "tweet_id_and_sha256_fingerprint",
  "queries_file": "config/search_queries.txt"
}
```

Do not claim that 2,000 real posts were collected until the final dataset and collector output confirm the target.
